...
import com.voxeet.uxkit.activities.Chat_screen;
...
public class VoxeetActionBarView extends VoxeetView {
    ...
    private boolean chatScreen = true;
    private boolean chatButton = false;

    private ImageView microphone;
    private ImageView speaker;
    private ImageView camera, chat, redTag;
    private ImageView hangup;
    private ImageView screenshare;

    private ViewGroup microphone_wrapper;
    private ViewGroup speaker_wrapper;
    private ViewGroup camera_wrapper, chat_wrapper;
    ...
    String conferenceID;
    private Boolean backIntent = false;
    ...
    public VoxeetActionBarView setChat(boolean chat) {
    ...
    protected void onAttachedToWindow() {
        ...
        if (null != VoxeetSDK.conference()) {
            ...
            updateCameraState();
            conferenceID = service.getConferenceId();
        }
    }
    ...
    protected void bindView(View v) {
        ...
        hangup.setOnClickListener(v1 -> {
            ...
            Chat_screen.mMessages = new ArrayList<>();
            if (Chat_screen.chatContext != null) {
                Chat_screen.chatContext.finish();
                Chat_screen.chatContext = null;
            }
        ...
        chat_wrapper = v.findViewById(R.id.chat_wrapper);
        redTag = v.findViewById(R.id.red_tag);
        chat.setEnabled(false);
        chat.setOnClickListener(v15 -> {
            backIntent = true;
            VoxeetToolkit.instance().getConferenceToolkit()
                    .setDefaultOverlayState(OverlayState.MINIMIZED);
            redTag.setVisibility(GONE);
            getContext().startActivity(new Intent(getContext(), Chat_screen.class).putExtra("ID", conferenceID));
        });
    ...
    @NonNull
    public VoxeetActionBarView setChatButton(boolean displayButton) {
        this.chatButton = displayButton;
        if(chatButton){
            chat.setImageDrawable(getResources().getDrawable(R.drawable.chat_white));
        }else {
            chat.setImageDrawable(getResources().getDrawable(R.drawable.chat_black));
        }
        chat.setEnabled(chatButton);
        if (backIntent && !chatButton) {
            backIntent = false;
            VoxeetToolkit.instance().getConferenceToolkit()
    ...
    public void onEvent(MessageReceived event) {
        if (event.message != null && !event.message.isEmpty()) {
            try {
                ...
                if (Chat_screen.chatContext == null) {
                    Chat_screen.mMessages.add(new MessageModel("" + System.currentTimeMillis() / 1000, jsonObject.optString("content"), false, jsonObject.optString("type"), jsonObject.optString("avatarUrl"), jsonObject.optString("ownerId"), jsonObject.optString("attachmentUrl"), jsonObject.optString("name")));
                }
                Log.e("overlaystate ", " state " + VoxeetToolkit.instance().getConferenceToolkit().getCurrentOverlayState());
                if (VoxeetToolkit.instance().getConferenceToolkit().getDefaultOverlayState().equals(OverlayState.EXPANDED)) {
                    redTag.setVisibility(VISIBLE);
                } else if (Chat_screen.chatContext == null) {
                    redTag.setVisibility(VISIBLE);
                } else {
                    redTag.setVisibility(GONE);
                }
    ...
    private void setUserPreferences() {
        ...
        if (chat != null)
            chat_wrapper.setVisibility(chatScreen && !listener ? VISIBLE : GONE);
